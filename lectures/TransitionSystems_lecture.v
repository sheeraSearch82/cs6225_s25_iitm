...
Proof.
  repeat econstructor.
  (* [econstructor]: tries all declared rules of 
   * the predicate in the conclusion, attempting 
   * each with [eapply] until one works. *)

  (* Note that here [econstructor] is doing double duty, 
   * applying the rules of both [trc] and [fact_step]. *)
Qed.
...
Theorem fact_invariant_ok : forall original_input,
  invariantFor (factorial_sys original_input) 
...
Lemma fact_invariant_always : forall original_input s,
  reachable (factorial_sys original_input) s
  -> fact_invariant original_input s.
(* Needed for the final theorem *)
Proof.
  simplify.
  Check use_invariant.
  eapply use_invariant.
