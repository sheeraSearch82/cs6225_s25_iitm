...
| Var (x : var)
| Plus (e1 e2 : arith)
| Minus (e1 e2 : arith) (* new addition *)
| Times (e1 e2 : arith).
...
Fixpoint interp (e : arith) (v : valuation) : nat :=
  ...
  | Var x =>
    ...
    match v $? x with
    | None => 0 (* goofy default value! Our terms may not be closed. *)
    | Some n => n
    end
...
(* Unfortunately, we can't execute (using [Compute]) code based on 
 * finite maps, since, for convenience of proving, they use 
 * uncomputable features. *)

Check fmap.
(* Need generic hashing and comparison functions for *ANY* Coq type.
 * Impossible! *)

(* But can prove theorems about execution *)
...
Example ex3 := Plus (Times (Var "x") (Const 5)) (Var "y").
               (* x * 5 + y *)
...
Fixpoint selfCompose {A} (f : A -> A) (n : nat) (x : A) : A :=
  match n with
  | O => x
  | S n' => selfCompose f n' (f x)
  end.
...
Proof.
  ...
  all: clear IHinput.
  (* [clear] deletes a hypothesis from the assumptions *)
...
Proof.
  simplify. (* simplify does the intros as well! *)
  rewrite H.
