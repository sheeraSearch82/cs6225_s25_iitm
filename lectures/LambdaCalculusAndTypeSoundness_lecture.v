...
Module Ulc.
  ...
  (* Any closed term is evaluated to a value *)
  ...
  (* Believe it or not, this is a Turing-complete language!  
     Here's an example nonterminating program. *)
  ...
  (* By the way: since [canonical'] doesn't mention variable "m", 
     substituting for "m" has no effect.  This fact will come in 
     handy shortly. *)
  ...
  (* Skipping the same exercise for multiplication. *)
  ...
  (* We can also port to this setting our small-step semantics 
     style. *)

  (* Function application (called "beta reduction") is the big 
     rule here. *)
  ...
  (* However, we also need bureaucractic/administrative rules 
     for pushing evaluation inside applications. *)
  ...
  (* Here we now go through a proof of equivalence between 
     big- and small-step semantics, though we won't spend 
     any further commentary on it. *)
...
Module Stlc.
  ...
  (* Then we have a bunch of bureaucratic, repetitive rules 
     encoding evaluation order. See "More Evaluation Contexts" 
     lecture for how to streamline this part, but for now note
     that the [value] premises below are crucial to enforce a 
     single order of evaluation. *)
  | App1 : forall e1 e1' e2,
      step e1 e1'
      -> step (App e1 e2) (App e1' e2)
  | App2 : forall v e2 e2',
  ...
  (* That language is suitable to describe with a static *type system*.  
     Here's our modest, but countably infinite, set of types. *)
  Inductive type :=
  | Nat                  (* Numbers *)
  ...
  Lemma progress : forall e t,
    ...
    -> unstuck e.
  Proof.
    (* we will walk through some of this *)
  ...
  (* Let's do preservation next. We need lemmas [weakening] and 
     [substitution] for this. *)

  (* Inclusion between typing contexts is preserved by adding the 
     same new mapping to both. *)
