...
(* Here's some appropriate syntax for expressions 
   (side-effect-free) of a simple imperative language 
   with a mutable memory. *)
Inductive exp :=
| Const (n : nat)
| Var (x : string)
| Read (e1 : exp)   (* Note *)
...
(* Here's the syntax of side-effecting commands, where we 
   attach an assertion to every "while" loop, for reasons 
   that should become clear later.  The assertion is ignored 
...
(* Shorthand notation for looking up in a finite map, returning zero if
   the key is not found *)
Notation "m $! k" := (match m $? k with Some n => n | None => O end) (at level 30).
(* Used for "All heap locations initialised to 0" *)
...
(* Assertions execute only when they are true.  They provide 
   a way to embed proof obligations within programs. *)
...
(* Here's an inductive predicate capturing a class of *proved* 
   specifications for commands.  The intuition is that, when 
   [hoare_triple P c Q], we know that, when we start [c] in a 
   state satisfying [P], if [c] finishes, its final state satisfies 
   [Q]. *)
...
| HtAssign : forall (P : assertion) x e,
  hoare_triple P 
    (Assign x e) 
    (fun h v => exists v', P h v' 
                           /\ v = v' $+ (x, eval e h v'))
| HtWrite : forall (P : assertion) (e1 e2 : exp),
  hoare_triple P 
    (Write e1 e2) 
    (fun h v => exists h', P h' v 
                /\ h = h' $+ (eval e1 h' v, eval e2 h' v))
...
| HtWhile : forall (I P : assertion) b c,
  ...
  -> hoare_triple P (While_ I b c) 
                    (fun h v => I h v /\ beval b h v = false)
...
Notation "h & v ~> e" := 
  (fun h v => e%nat%type) (at level 85, v at level 0).
...
Ltac t := cbv beta; propositional; subst;
          ...
          repeat match goal with
                 ...
                 | [ H : ?E = ?E |- _ ] => clear H
                 end; simplify; propositional; auto; try equality; 
...
Theorem swap_ok : forall a b,
   {{_&v ~> v $! "x" = a /\ v $! "y" = b}}
    "tmp" <- "x";;
(* {{_ & v ~> (exists v' : valuation,
                 (v' $! "x" = a /\ v' $! "y" = b) /\
                 v = v' $+ ("tmp", a))}} *)
    "x" <- "y";;
(* {{_ & v ~> (exists v' : valuation,
              (exists v'0 : valuation,
                 (v'0 $! "x" = a /\ v'0 $! "y" = b) /\
                  v' = v'0 $+ ("tmp", a)) /\
                  v  = v'  $+ (  "x", b))}} *)
    "y" <- "tmp"
...
(* Finally, our main inductive proof: 
    
    small steps preserve the existence of Hoare triples.  
   
   We even give the concrete specification for the new command
   [c'] that was stepped to.  It keeps the old postcondition, 
   and we give it a very specific precondition saying 
   "the state is exactly this." *)
...
Proof.
  ...
  cases p0.
  simplify.
  eapply hoare_triple_step; (* preservation *)
    eauto.
  simplify; auto.
  ...
  cases p.
  simplify.
  eapply hoare_triple_unstuck; (* progress *)
    eauto.
  simplify; auto.
Qed.
...
Theorem forever_invariant : 
  invariantFor (trsys_of ($0, $0, forever)) unstuck.
Proof.
