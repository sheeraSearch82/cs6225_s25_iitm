...
Make sure you read and understand `LambdaCalculusAndTypeSoundness.v` from FRAP,
since a lot of the proof structure from that file can be reused in this Pset!
We certainly aren't expecting you to reconstruct the whole theory of type
soundness from scratch.  Then, the follow-on file `EvaluationContexts.v` is an
even better source of examples to copy from, since this pset adopts the
evaluation-contexts technique exercised thoroughly by that file/chapter.
