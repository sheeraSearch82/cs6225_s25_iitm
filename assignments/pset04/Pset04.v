(** CS6225 -- Problem Set 4 (100 points) *)

(** * 6.512 Formal Reasoning About Programs, Spring 2023 - Pset 3 *)

Require Import Frap ZArith.
Require Import Pset04Sig.
...
Module Impl.
  ...
  *)
  ...
  (* As usual, the grading rubric for this pset is in Pset04Sig.v. *)
  ...
  (* use [unfold] tactic from lectures to unfold the definitions of functions 
     here. *)
  ...
  (* Here's an example: The function that subtracts two from its argument
     ...
     [linear_arithmetic] tactic may come in handy. *)
  ...
  (* HINT: If you want to apply a lemma that binds terms that don't appear
     ...
     With multiple forall-bound names, you can do
                   
                   apply <theorem name> with (<x>:=<term1>) (<y>:=<term2>)
                   
     If you are [apply]ing to one of the assumptions, and you want a fresh
     assumption as a result, you can do
     
         apply <theorem name> with (<x>:=<term1>) in <H> as <newH>
     
     where <newH> is the name of the new assumption. The original assumption
...
Module ImplCorrect : Pset04Sig.S := Impl.
