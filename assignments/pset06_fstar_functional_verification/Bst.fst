...
(* TODO (10 points): Define the function [tree_max] (it finds the largest
 * element in a tree) and refine its type so that the function delete (below, in
 * comments) type-checks.
 *
 * Observe that [tree_max] is a total function that returns an int. Hence, it
 * cannot accept [Leaf] as an argument. In F*, whenever you define a data type,
 * you get boolean discriminators for constructors for free. For example: *)
